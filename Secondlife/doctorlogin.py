    ...
    <div class="container">
        ...
        <form method="post">
            ...
            <div class="form-group">
                <input type="text" class="form-control" name="DOCTOR" required placeholder="Doctor ID">
            </div>
            <div class="form-group">
                <input type="password" class="form-control" name="PASS" required placeholder="Password">
            </div>
            <div class="form-link">
                <a href="forgetpassword.py?id=1">Forgot Password?</a>
            </div>
...
did = form.getvalue("DOCTOR")
password = form.getvalue("PASS")
submit = form.getvalue("SUB")
if submit != None:
    q = """select id from doctor where DoctorID='%s' and Password='%s'""" % (did, password)
    cur.execute(q)
    idd = cur.fetchone()
    print("""
    <script>
    alert("Login success");
    location.href="doctorindex.py?id=%s";
    </script>
    """ % (idd))
