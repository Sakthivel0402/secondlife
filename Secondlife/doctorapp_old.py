...
<head>
    ...
    <title>Doctor Dashboard - Completed Appointments</title>
...
if res[0][13] != "aware":
    l = """Select * from addicted where doctor='%s' and status='Completed'""" % (res[0][1])
    cur.execute(l)
    add = cur.fetchall()

    print("""
                <div class="table-container">
                    <h2 style="color: var(--primary-color);">Completed Patient Appointments</h2>
    ...
    for i in add:
        ...
        <tr>
            ...
            <td>{i[4]}</td>
            <td>{i[5]}</td>
            <td>{i[12] if i[12] else 'N/A'}</td>
            <td>
        """)

        b = """select * from chat where patient='%s' and doctor='%s'""" % (i[1], i[2])
        cur.execute(b)
        d = cur.fetchall()
        di = d[0][5] if d else "0"

        if di != "0":
            print(f"""
                <a href="docchat.py?id={res[0][0]}&id2={i[0]}" class="btn btn-success btn-chat position-relative">
                    <i class="bi bi-chat-left-text"></i> Chat
                    <span class="chat-badge">{di}</span>
                </a>
            """)
        else:
            print(f"""
                <a href="docchat.py?id={res[0][0]}&id2={i[0]}" class="btn btn-success">
                    <i class="bi bi-chat-left-text"></i> Chat
                </a>
            """)
...
if res[0][13] != "Physician":
    n = """Select * from aware where doctor='%s' and status='Completed'""" % (res[0][1])
    cur.execute(n)
    aware = cur.fetchall()

    print("""
                <div class="table-container mt-4">
                    <h2 style="color: var(--primary-color);">Completed Aware Patient Appointments</h2>
                    <div class="table-responsive">
                        <table class="table table-hover">
                            <thead class="table-primary">
                                <tr>
    ...
    for i in aware:
        ...
        if di != "0":
            print(f"""
                <a href="docchat.py?id={res[0][1]}&id2={i[1]}" class="btn btn-success btn-chat position-relative">
                    <i class="bi bi-chat-left-text"></i> Chat
                    <span class="chat-badge">{di}</span>
                </a>
            """)
            ...
            print(f"""
                <a href="docchat.py?id={res[0][1]}&id2={i[1]}" class="btn btn-success">
                    <i class="bi bi-chat-left-text"></i> Chat
                </a>
            """)
