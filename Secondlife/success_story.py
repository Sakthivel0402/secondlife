...
<label></label>
<form method="post">
<textarea  class="form-control" name="FEED" rows="5" cols="33">
...
<div class="col-md-12">
<label></label>
<input type="Submit" class="btn btn-primary mt-3"x name="SUB">  
</div> 
</div>
</form>"""%(res[0][1]))
sub=form.getvalue("SUB")
succsto=form.getvalue("FEED")
Name=form.getvalue("NAME")
if sub!= None:
    b=f"""insert into succsto(Name,Feed) values('{Name}','{succsto}')"""
    cur.execute(b)
    con.commit()
    print("""  
    <script>
    alert("Story Shared");
    location.href="patient_com.py?id=%s";
    </script>
    """%(did))
print("""
</div>
